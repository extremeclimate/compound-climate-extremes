...
DATA_PATHS = {
    "Duration": {
        "HEAT": os.path.join(DATA_DIR, "duration", "heat.csv"),
        "COLD": os.path.join(DATA_DIR, "duration", "cold.csv"),
        "RAIN": os.path.join(DATA_DIR, "duration", "heavy_rain.csv"),
        "DROUGHT": os.path.join(DATA_DIR, "duration", "drought.csv"),
        "WIND": os.path.join(DATA_DIR, "duration", "strong_wind.csv"),
        "WIND_DROUGHT": os.path.join(DATA_DIR, "duration", "wind_drought.csv")
    },
    "Intensity": {
        "HEAT": os.path.join(DATA_DIR, "intensity", "heat.csv"),
        "COLD": os.path.join(DATA_DIR, "intensity", "cold.csv"),
        "RAIN": os.path.join(DATA_DIR, "intensity", "heavy_rain.csv"),
        "DROUGHT": os.path.join(DATA_DIR, "intensity", "drought.csv"),
        "WIND": os.path.join(DATA_DIR, "intensity", "strong_wind.csv"),
        "WIND_DROUGHT": os.path.join(DATA_DIR, "intensity", "wind_drought.csv")
    }
}

MAP_CONFIGS = {
    "Duration": [
        {'prefix': 'HEAT', 'cmap': 'YlOrRd', 'title': 'a) Heat Wave (Duration, Days)', 'vmax': 10.0},
        {'prefix': 'COLD', 'cmap': 'Blues', 'title': 'b) Cold Wave (Duration, Days)', 'vmax': 10.0},
        {'prefix': 'RAIN', 'cmap': 'GnBu', 'title': 'c) Heavy Rainfall (Duration, Days)', 'vmax': 6.0},
        {'prefix': 'DROUGHT', 'cmap': 'OrRd', 'title': 'd) Drought (Duration, Months)', 'vmax': 3.0},
        {'prefix': 'WIND', 'cmap': 'YlGn', 'title': 'e) Strong Wind (Duration, Days)', 'vmax': 8.0},
        {'prefix': 'WIND_DROUGHT', 'cmap': 'BuPu', 'title': 'f) Wind Drought / Stagnation (Duration, Days)',
         'vmax': 6.0}
    ],
    "Intensity": [
        {'prefix': 'HEAT', 'cmap': 'YlOrRd', 'title': 'a) Heat Wave (Intensity)', 'vmax': 4.0},
        {'prefix': 'COLD', 'cmap': 'Blues', 'title': 'b) Cold Wave (Intensity)', 'vmax': 2.0},
        {'prefix': 'RAIN', 'cmap': 'GnBu', 'title': 'c) Heavy Rainfall (Intensity)', 'vmax': 0.15},
        {'prefix': 'DROUGHT', 'cmap': 'OrRd', 'title': 'd) Drought (Intensity)', 'vmax': 1.0},
        {'prefix': 'WIND', 'cmap': 'YlGn', 'title': 'e) Strong Wind (Intensity)', 'vmax': 1.5},
        {'prefix': 'WIND_DROUGHT', 'cmap': 'BuPu', 'title': 'f) Wind Drought / Stagnation (Intensity)', 'vmax': 0.8}
    ]
}

def load_spatial_base():
    print("\n[INIT] Loading foundational geographic data (Shapefiles)...")
    ...
    print("[INIT] Simplifying polygons to turbocharge rendering...")
    gdf_cities['geometry'] = gdf_cities['geometry'].simplify(tolerance=10000, preserve_topology=True)

    return gdf_cities, gdf_world_correct_china

def run_pipeline_for_metric(metric_name, gdf_cities_base, gdf_world_correct_china):
    print(f"\n{'=' * 50}\n STARTING PIPELINE FOR: {metric_name.upper()}\n{'=' * 50}")

    print(f"[{metric_name}] Loading CSV datasets...")
    paths = DATA_PATHS[metric_name]

    df_heat = pd.read_csv(paths["HEAT"])
    df_cold = pd.read_csv(paths["COLD"])
    df_rain = pd.read_csv(paths["RAIN"])
    df_drought = pd.read_csv(paths["DROUGHT"])

    df_wind = pd.read_csv(paths["WIND"], index_col=0).rename_axis('NAME_2').reset_index()
    df_wind_drought = pd.read_csv(paths["WIND_DROUGHT"], index_col=0).rename_axis('NAME_2').reset_index()
    ...
    print(f"[{metric_name}] Aggregating decadal means...")
    ...
    print(f"[{metric_name}] Merging with spatial features...")
    gdf_plot = gdf_cities_base.merge(agg_heat, on='NAME_2', how='left') \
    ...
    print(f"[{metric_name}] Firing up rendering engine...")
    ...
    rows_config = MAP_CONFIGS[metric_name]
    ...
    with tqdm(total=total_maps, desc=f" Rendering {metric_name} Maps",
    ...
    output_tif_no_bounds = os.path.join(
        OUTPUT_DIR, f"Figure_Appendix_6x4_{metric_name}_NoBounds_correct_china_{timestamp}.tif"
    )
    output_svg_no_bounds = os.path.join(
        OUTPUT_DIR, f"Figure_Appendix_6x4_{metric_name}_NoBounds_correct_china_{timestamp}.svg"
    )

    total_save_start = time.time()

    print(f"[{metric_name}] Saving TIFF (300 DPI, LZW compressed)...", flush=True)
    tif_start = time.time()
    plt.savefig(output_tif_no_bounds, dpi=300, bbox_inches='tight', facecolor='white',
                pil_kwargs={"compression": "tiff_lzw"})
    print(f"[{metric_name}] TIFF saved successfully! Time used: {(time.time() - tif_start) / 60:.2f} min", flush=True)
    print(f"[{metric_name}] TIFF path: {output_tif_no_bounds}", flush=True)

    print(f"[{metric_name}] Saving SVG...", flush=True)
    svg_start = time.time()
    plt.savefig(output_svg_no_bounds, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"[{metric_name}] SVG saved successfully! Time used: {(time.time() - svg_start) / 60:.2f} min", flush=True)
    print(f"[{metric_name}] SVG path: {output_svg_no_bounds}", flush=True)
    print(f"[{metric_name}] Total saving time: {(time.time() - total_save_start) / 60:.2f} min", flush=True)

    plt.close(fig)
    print(f" {metric_name} Pipeline Completed Successfully!\n")

if __name__ == "__main__":

    gdf_cities_base, gdf_world_correct_china = load_spatial_base()

    for metric in ["Duration", "Intensity"]:
        run_pipeline_for_metric(metric, gdf_cities_base, gdf_world_correct_china)

    print(f"ALL DONE! Duration and Intensity were each saved as 300 DPI LZW TIFF + SVG in: {OUTPUT_DIR}")
